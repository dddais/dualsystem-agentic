...
def main():
    ...
    with tempfile.TemporaryDirectory() as directory:
        if integrated:
            ...
            runtime.recorder = ProgressRecorder(runtime, output_dir=Path(directory) / "recordings",
                                               scheduler_client=scheduler, poll_interval_s=.05)
        with serve(create_app(runtime)) as runtime_url:
            ...
            with serve(monitor_app(backend)) as monitor_url, tempfile.TemporaryFile(mode="w+") as log:
                ...
                try:
                    with sync_playwright() as playwright:
                        ...
                        if integrated:
                            # Refreshing the browser must not end recording or create a second session.
                            expect(page.locator("#bridge-record")).to_have_text("停止录制（录制中）")
                            assert len(runtime.recorder._sessions) == 1
                            page.locator("#bridge-record").click()
                            expect(page.locator("#progress-recording-status")).to_contain_text("记录已保存", timeout=10000)
                            download_link = page.locator("#progress-recording-download")
                            expect(download_link).to_be_visible()
                            with page.expect_download() as downloading:
                                download_link.click()
                            archive_path = args.screenshot.replace(".png", "-progress.zip")
                            downloading.value.save_as(archive_path)
