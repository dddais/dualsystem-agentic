...
def test_build_online_robot_app_wires_components_from_config():
    ...
    assert app.runtime.reason_interval_s == 1.0
    assert app.runtime.monitor_poll_interval_s == 1.0
    assert app.runtime.max_monitor_polls == 300


def test_build_online_robot_app_wires_reason_interval_from_config():
    config = _scripted_config()
    config.loop.reason_interval_s = 0.25

    app = build_online_robot_app(config)

    assert app.runtime.reason_interval_s == 0.25
