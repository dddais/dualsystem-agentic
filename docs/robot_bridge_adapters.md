...
1. 选择模板并填目标，或输入完整 instruction，点击“保存指令”；保存本身不启动机器人或 Monitor。
...
已保存指令可跨页面刷新和 loop 重启复用；保存在 Runtime 内存中，重启 Runtime 后需重新保存。
...
单步属于执行内部的操作，而空闲会结束本轮任务。本轮 instruction 固定，编辑区可保存下一轮指令；
...
`prompt_map` 替换。配置使用 `prompt_mode: text`；Scheduler 需已支持此前新增的 `set_prompt_text` 接口。
