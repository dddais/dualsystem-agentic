...
`events.jsonl` 中记录 run/session/step 事件；step 事件包含 VLM 输入、渲染后的 prompt、
VLM 原始输出、解析结果、工具结果、executor 结果、monitor 状态和停止原因。图片会保存
为文件，JSONL 只保存路径、hash 和大小引用，不直接嵌入 base64。
