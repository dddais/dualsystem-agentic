...
loop:
  ...
  reason_interval_s: 1.0     # VLM 主循环推理周期
  monitor_poll_interval_s: 1.0 # legacy monitor 轮询周期
...
`reason_interval_s` 控制没有事件触发时 VLM 多久做一次主循环推理；
`monitor_poll_interval_s` 控制异步 monitor 兼容轮询，两者可以独立调整。
...
可以把一个 run 中的每个 session 分别渲染成带标注的三视角 MP4，视频中会显示
controller phase、当前 subtask、monitor 状态、VLM/monitor-only step、事件和
subtask 时间线：
...
使用 `--session <session_id>` 只渲染一个长程任务；使用 `--combine-run` 才会生成
整次 run 的总览视频。
